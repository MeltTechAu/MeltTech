...
var selected_profile_name = 'AutoTune.json';
var temp_scale = "c";
...
function updateProfile(id)
{
    ...
    var job_seconds = timeProfileFormatter(
            ...
            false);
    var kwh = (oven_power*job_seconds/60/1000).toFixed(2);
    var cost =  (kwh*kwh_rate).toFixed(2);
    var job_time = new Date(job_seconds * 60000).toISOString().substr(11, 8);
...
function timeProfileFormatter(val, down) {
    ...
    switch(time_scale_profile){
        case "m":
            if (down) {rval = val * 1} else {rval = val * 1}
            break;
        case "h":
...
function initDatetimePicker() {
    ...
    inOneMinutes.setMinutes(inOneMinutes.getMinutes() + 10);
    $('#scheduled-run-time').attr('value', formatDateInput(inOneMinutes));
